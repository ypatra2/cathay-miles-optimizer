...
"""
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            # Extract patch summary from the last line (using simple search backwards)
            ...
            try:
                ...
                deltas_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "engine_deltas")
                os.makedirs(deltas_dir, exist_ok=True)
                with open(os.path.join(deltas_dir, f"failed_patch_attempt_{attempt + 1}.py"), "w") as f:
                    f.write(cleaned)

                if attempt < MAX_RETRIES - 1:
                    # Append error feedback and retry
                    error_msg = f"\n\n## COMPILATION FAILED\nThe code you just generated failed to compile with this error:\n```\n{str(e)}\n```\nLine {getattr(e, 'lineno', 'unknown')}: {getattr(e, 'text', '')}\n\nYou likely left a string unterminated or missed a bracket. Please fix the error and output the FULL CORRECTED code again."
                    current_prompt += error_msg
                    continue
                else:
                    return {
                        "patch_status": "failed",
                        "error": f"Generated code has syntax error after {MAX_RETRIES} attempts: {str(e)}",
                        "proposed_patch": cleaned,
                        "patch_summary": "FAILED: Syntax error in generated code",
                    }

            # If we get here, compile succeeded. Save to engine_deltas/ and return.
            month_tag = datetime.now().strftime("%Y-%m")
            deltas_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "engine_deltas")
            os.makedirs(deltas_dir, exist_ok=True)
            ...
            return {
                "patch_status": "failed",
                "error": f"Dev Agent API request failed: {str(e)}",
            }
